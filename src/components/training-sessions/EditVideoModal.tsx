...
export function EditVideoModal({ isOpen, onClose, videoData, onSuccess }: EditVideoModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (videoData) {
        setFormData({
          Select_category: videoData.Select_category || "",
          Title: videoData.Title || "",
          Subtitle: videoData.Subtitle || "",
          Equipment_used: videoData.Equipment_used || "",
          Steps: videoData.Steps || "",
          Time: videoData.Time || "",
          Points: videoData.Points?.toString() || ""
        });
      }
      setVideoFile(null);
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${window.location.origin}/api/admin/videos/${videoData.id}/`, {
        method: 'PATCH',
        headers: {
      ...
      alert(`Failed to update video: ${e}`);
    } finally {
            ...
            <div style={{ position: 'relative' }}>
              ...
              >
                ...
                <p style={{ margin: 0, fontSize: '13px', color: 'var(--text-primary)' }}>
                  {videoFile ? videoFile.name : 'Drag and drop or click to replace video (MP4, MOV)'}
                </p>
